...
$tourPriceBy = get_post_meta($post_id,'tour_price_by',true);
    ...
    <?php endif; ?>
        ...
        <div class="form-content clearfix">
            ...
            <div style="margin-bottom: 15px" class="row">
                ...
                <?php if( $tourPriceBy == "fixed" ) { if( get_post_type($post_id) == 'st_tours' || get_post_type($post_id) == 'st_activity' ){ ?>

                    <div class="col-xs-12">                        
                        <table class="grp-price-bulk-table">
                            <tr>
                                <th>Group Name</th>
                                <th>Group Min Size</th>
                                <th>Group Max Size</th>
                                <th>Group Price</th>
                            </tr>
                            <tr>
                                <td><input type="text" name="price-bulk[]['grp-name']" class="grp_name"></td>
                                <td><input type="number" min="0" name="price-bulk[]['grp-min-size']" class="grp_min_size"></td>
                                <td><input type="number" max="100" name="price-bulk[]['grp-max-size']" class="grp_max_size"></td>
                                <td><input type="text" name="price-bulk[]['grp-price']" class="grp_price"></td>
                            </tr>
                            <tr>
                                ...
                                <td style="text-align: center;" colspan="2">
                                    <button class="del-row">Delete Row</button>
                                </td>
                            </tr>
                        </table>
                    ...
                    <style>
                        .grp-price-bulk-table {
                            table-layout: fixed;
                            width: 100%;
                        }
                        .grp-price-bulk-table tr td input {
                            width: 100%;
                        }
                    </style>
                    <script>
                        jQuery(".grp-price-bulk-table .add-row").on("click",function(e){
                            e.preventDefault();
                            $newRow = jQuery(".grp-price-bulk-table tr:eq(1)").html();
                            jQuery(".grp-price-bulk-table tr:last").before("<tr>"+$newRow+"</tr>");
                        });
                        jQuery(".grp-price-bulk-table .del-row").on("click",function(e){
                            e.preventDefault();
                            $rowLength =  jQuery(".grp-price-bulk-table tr").length;
                            if( $rowLength > 3 ) {
                                jQuery(".grp-price-bulk-table tr:eq("+($rowLength-2)+")").remove();
                            }
                        });
                    </script>
            ...
            <?php if(get_post_type($post_id) == 'st_tours' || get_post_type($post_id) == 'st_activity'){ ?>

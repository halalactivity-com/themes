    ...
    if ( !class_exists( 'STActivity' ) ) {
        ...
        class STActivity extends TravelerObject
        {
            ...
            function _get_where_query( $where )
            {
                ...
                // echo '<pre>' . var_export(explode(",", $list_date), true) . '</pre><br/>';
                // die();
                ...
                if ( $check = STInput::get( 'orderby' ) ) {
                    ...
                    switch ( $check ) {
                        ...
                        case "old":
                            ...
                            break;
                        case "review_asc":
                            $orderby = ' rate_review asc';
                            break;
                        case "review_desc":
                            $orderby = ' rate_review desc';
                            break;
                    }
                }
            ...
            function do_add_to_cart()
            {
                ...
                $tour_price_by = get_post_meta($post_origin, 'tour_price_by', true);
                $number                  = 1;
                ...
                if ( !$check_in || !$check_out ) {
                    STTemplate::set_message( __( 'Select a date from the calendar.', ST_TEXTDOMAIN ), 'danger' );
                    $pass_validate = false;
                ...
                if ( $period < $booking_period ) {
                    STTemplate::set_message( sprintf( __( 'This activity allows booking for a minimum of %d day(s)', ST_TEXTDOMAIN ), $booking_period ), 'danger' );
                    $pass_validate = false;
                ...
                if($min_number > 0){
                    if ( $adult_number + $child_number + $infant_number < $min_number ) {
                        STTemplate::set_message( sprintf( __( 'Min number of people for this activity is %d people', ST_TEXTDOMAIN ), $min_number ), 'danger' );
                        $pass_validate = false;
                ...
                if ( $max_number > 0 ) {
                    if ( $adult_number + $child_number + $infant_number > $max_number ) {
                        STTemplate::set_message( sprintf( __( 'Max number of people for this activity is %d people', ST_TEXTDOMAIN ), $max_number ), 'danger' );
                        $pass_validate = false;
                ...
                if ( !$tour_available ) {
                    STTemplate::set_message( __( 'This activity is not available for this day', ST_TEXTDOMAIN ), 'danger' );
                    $pass_validate = false;

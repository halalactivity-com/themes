    ...
    class AvailabilityHelper
    {
        ...
        public function traveler_calendar_bulk_edit_form()
        {
            ...
            if ($post_id > 0) {
                ...
                if (isset($all_days) && !empty($all_days)) {
                ...
                $price = stripslashes(STInput::post('price_bulk', 0));
                ...
                if (!($price) || !is_numeric($adult_price) || !is_numeric($children_price) || !is_numeric($infant_price)) {
                    echo json_encode([
                        'status' => 0,
                        'message' => '<div class="text-error">' . $price . __('The price field is not a number.', ST_TEXTDOMAIN) . '</div>'
                    ]);
                    die;
                }
            ...
            if ($data['groupday'] == 0) {
                for ($i = $start; $i <= $end; $i++) {
                    ...
                    if (!isset($data['starttime']))
                        $data['starttime'] = '';
                    $this->traveler_insert_availability($data['post_id'], $data['start'], $data['end'], stripslashes($data['price']), $data['adult_price'], $data['children_price'], $data['infant_price'], $data['starttime'], $data['status'], $data['groupday'], $table);
                }
            } else {
                for ($i = $start; $i <= $end; $i++) {
        ...
        public function _get_availability_activity()
        {
            ...
            if (get_post_type($activity_id) == 'st_activity') {
                ...
                if (!empty($availability)) {
                    foreach ($availability as $key => $val) {
                        $status = $val['status'];
                        $basePrice = json_decode( stripslashes( $val['price'] ) );
                        if (intval($val['groupday']) == 1) {
                            $return[] = [
                                'start' => date('Y-m-d', $val['check_in']),
                                'date' => date('Y-m-d', $val['check_in']),
                                ...
                                'date_end' => date('d', $val['check_out']),
                                'status' => $status,
                                'base_price' => $basePrice,//((float)$val['price'] < 0) ? 0 : (float)$val['price'],
                                'adult_price' => ((float)$val['adult_price'] < 0) ? 0 : (float)$val['adult_price'],
                                'child_price' => ((float)$val['child_price'] < 0) ? 0 : (float)$val['child_price'],
                                'infant_price' => ((float)$val['infant_price'] < 0) ? 0 : (float)$val['infant_price'],
                                'starttime' => $val['starttime'],
            ...
            if (get_post_type($activity_id) == 'st_activity') {
                ...
                if (!empty($availability)) {
                    foreach ($availability as $key => $val) {
                        ...
                        $price_type = get_post_meta($activity_id, 'tour_price_by', true);
                        ...
                        if (intval($val['groupday']) == 1) {
                            ...
                            $ev = '';
                            if( $price_type == "fixed" ) {
                                $groupObj = json_decode($val['price']);
                                foreach ($groupObj as $key => $grp) {
                                    $grp_init_price = $grp->grp_price;
                                    break;
                                }
                                $ev = __('Group Price: ', ST_TEXTDOMAIN) . TravelHelper::format_money($grp_init_price) . '<br/>';
                            }
                            else {
                                if ($hideA != 'on') {
                                    $ev = __('Adult: ', ST_TEXTDOMAIN) . TravelHelper::format_money($val['adult_price']) . '<br/>';

    ...
    class STPrice{
        ...
        static function getPriceByPeopleTour($tour_id = '', $check_in = '', $check_out = '', $adult_number = 0, $child_number = 0, $infant_number = 0){
            ...
            if($post_type == 'st_tours' || $post_type == 'st_activity'){
                ...
                $group_object = json_decode($people_price['price']);
                foreach ($group_object as $key => $grp) {
                    if( $group_size >= $grp->grp_min_size && $group_size <= $grp->grp_max_size ){
                        $group_price = $grp->grp_price;
                        break;
                    }
                }               
            }else{
            ...
            if($tour_price_by == 'fixed') {
                ...
                $data = array(
                    'adult_price' => round( $total_adult_price, 2 ),
                    'child_price' => round( $total_child_price, 2 ),
                    'infant_price' =>round( ($infant_number * $infant_price), 2 ),

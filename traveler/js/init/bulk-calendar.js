jQuery(document).ready(function ($) {
    ...
    function step_add_bulk(data1, posts_per_page, total, current_page, all_days, post_id, container, data_first) {
        ...
        $.post(ajaxurl, data, function (respon, textStatus, xhr) {
            console.log(respon); console.log( typeof(respon) );
            if (typeof(respon) == 'object') {
                if (respon.status == 2) {
                    console.log("This is the respon: ", respon );
    ...
    if ($('#form-bulk-edit').length) {
        $('#calendar-bulk-save').click(function (event) {
            ...
            var $grp_data = {};
            var keyName;
            jQuery('input[name^="price-bulk"]').each(function(key,value){
                if( jQuery(this).hasClass("grp_name") ) {
                    keyName = jQuery(this).val();
                    $grp_data[keyName] = {};
                }
                else {
                    $grp_data[keyName][jQuery(this).attr("class")] = jQuery(this).val();
                }
            });
            ...
            var data = {
                ...
                'months': months,
                'years': years,
                'price_bulk': JSON.stringify($grp_data),
